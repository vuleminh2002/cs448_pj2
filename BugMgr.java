...
public class BufMgr implements GlobalConst{
  ...
  public void pinPage(PageId pin_pgid, Page page, boolean emptyPage) throws BufferPoolExceededException {
    ...
    //System.out.println("[DEBUG pinPage] Called with pid=" + pin_pgid.pid +
    //        ", emptyPage=" + emptyPage);
    //first get the frameIndex
    int frameIndex = hashTable.lookup(pin_pgid.pid);
    System.out.println("[DEBUG pinPage]  frameindex=" + frameIndex + " pid " +pin_pgid.pid );

    // If the page exists.
    if (frameIndex != -1) {
      //System.out.println("page existed" + frameDescs[frameIndex].pageID.pid);
  ...
  public void unpinPage(PageId PageId_in_a_DB, boolean dirty) throws PageUnpinnedException, HashEntryNotFoundException {
    debugPrintFrames();
    System.out.println("Called unpin on pid " + PageId_in_a_DB.pid + " " + dirty);
    ...
    if (frameDescs[frameIndex].pin_count == 0) {
      freeList.addLast(frameIndex);  // Avoid duplicate entries in free list
    }
    System.out.println("Aftter unpin");
    debugPrintFrames();
  }
  ...
  public PageId newPage(Page firstpage, int howmany) {
    System.out.println("called newpage");
  ...
  public void freePage(PageId globalPageId) throws PagePinnedException {
    ...
    System.out.println("called freepage" + globalPageId.pid);
  ...
  public void flushPage(PageId pageid) {
    System.out.println("called flush page");
  ...
  public void flushAllPages() {
    ...
    //debugPrintFrames();
    System.out.println("call flush all page");
    for (int i = 0; i < numbufs; i++) {
